...
| **Policy bundles** (`.yaml`) | ✅ config | YAML (not Cedar files); one graph-kind bundle may be **shared across graphs**, while the one cluster-kind bundle is separate. A bundle never mixes `cluster` with graph refs, and each served scope has one bundle. Gates actions **and query exposure** (who may list/invoke each stored query) |
